...
class Maxwell_Pro_Header_Bar {
		...
		if ( has_nav_menu( 'social' ) or has_nav_menu( 'secondary' ) ) {
			...
			if ( has_nav_menu( 'secondary' ) ) : ?>
				...
				<div class="secondary-navigation">
					...
					<nav class="top-navigation" role="navigation" <?php self::amp_menu_is_toggled(); ?> aria-label="<?php esc_attr_e( 'Secondary Menu', 'maxwell-pro' ); ?>">
	...
	static function nav_menu_social_icons( $item_output, $item, $depth, $args ) {
		...
		// Get SVG.
		$svg = apply_filters( 'maxwell_pro_get_social_svg', self::get_social_svg( $icon ), $item_output );
	...
	static function get_social_svg( $icon = null ) {
		...
		$svg .= ' <use xlink:href="' . MAXWELL_PRO_PLUGIN_URL . 'assets/icons/social-icons.svg?ver=20221122#icon-' . esc_html( $icon ) . '"></use> ';
		$svg .= '</svg>';
	...
	static function supported_social_icons() {
		...
		return apply_filters( 'maxwell_pro_supported_social_icons', $supported_social_icons );
	}
