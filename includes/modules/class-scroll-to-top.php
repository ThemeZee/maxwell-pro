 ...
 * Scroll to top Class
 */
class Maxwell_Pro_Scroll_To_Top {
	...
	static function enqueue_script() {
		...
		if ( true === $theme_options['scroll_to_top'] && ! self::is_amp() ) :
			...
			wp_enqueue_script( 'maxwell-pro-scroll-to-top', MAXWELL_PRO_PLUGIN_URL . 'assets/js/scroll-to-top.min.js', array(), '20220924', true );

			// Passing Parameters to navigation.js.
			wp_localize_script( 'maxwell-pro-scroll-to-top', 'maxwellProScrollToTop', array(
				'icon'  => self::get_svg( 'collapse' ),
				'label' => esc_attr__( 'Scroll to top', 'maxwell-pro' ),
			) );
	...
	static function scroll_to_top_settings( $wp_customize ) {
		...
		$wp_customize->add_control( new Maxwell_Customize_Header_Control(
			$wp_customize, 'maxwell_theme_options[scroll_top_title]', array(
				'label'    => esc_html__( 'Scroll to top', 'maxwell-pro' ),
		...
		$wp_customize->add_setting( 'maxwell_theme_options[scroll_to_top]', array(
			'default'           => false,
			'type'              => 'option',
			'transport'         => 'refresh',
		...
		$wp_customize->add_control( 'maxwell_theme_options[scroll_to_top]', array(
			'label'    => __( 'Display Scroll to top button', 'maxwell-pro' ),
			'section'  => 'maxwell_pro_section_footer',
			'settings' => 'maxwell_theme_options[scroll_to_top]',
			'type'     => 'checkbox',
			'priority' => 20,
		) );
	}
...
// Run Class.
add_action( 'init', array( 'Maxwell_Pro_Scroll_To_Top', 'setup' ) );
